# SQS Queue for dmg-inbound-callrecording-persistence Lambda
...
resource "aws_sqs_queue_policy" "s3_to_sqs_persistence_policy" {
  queue_url = aws_sqs_queue.dmg_inbound_callrecording_persistence.id
    ...
    Statement = [
      {
        ...
        Resource = aws_sqs_queue.dmg_inbound_callrecording_persistence.arn
        Condition = {
          ArnEquals = {
